...
public class ProducerConsumer2010 extends JApplet
{
	...
	public static int pnlWidth = 250;	
	public ProducerConsumer2010( ) { init() ;} 
	public void stop() { }
	public void start() {
		for (int i = 0; i <  2; i++) manager.addProducer();
		for (int i = 0; i < 10; i++) manager.addConsumer();
	}
	public void init ()
	{
		...
		addComponentListener(
			new ComponentAdapter()
			{
				...
				{
					producerPanel.setSize(getWidth()/2-5, pnlWidth);
					consumerPanel.setSize(getWidth()/2-5, pnlWidth);
					validate();
				}
			}
		);
		...
		burgerTray.setSize(getWidth(), 80);
		burgerTray.setForeground(Color.red);

		burgerTray.setBackground(Color.lightGray);

		manager = new PCManager(this);
		producerPanel = new ProducerPanel(manager.producers);
		producerPanel.setSize(getWidth()/2-5, pnlWidth);
		producerPanel.setForeground(Color.blue);
		producerPanel.setBackground(Color.lightGray);
		
		consumerPanel = new ConsumerPanel(manager.consumers);
		consumerPanel.setSize(getWidth()/2-5, pnlWidth);
		consumerPanel.setForeground(Color.red);
		consumerPanel.setBackground(Color.lightGray);
		
		buttonPanel = new JPanel();
		buttonPanel.setSize(350, 40);	
	...
	public static void main(String[] args)
	{
		...
		ProducerConsumer2010 applet	= new ProducerConsumer2010() ;
		
		fm.addWindowListener( new WindowAdapter() {
			public void windowClosing ( WindowEvent e ) {
				System.exit(0); }} ) ;
		fm.getContentPane().add( applet ) ;
		fm.setTitle("Producer & Consumer Problem: Adaptive Burger Store");
		fm.setSize(800, 850);
...
class PCLib {
	...
	public static void drawOneLine(JPanel pn, Color c, int ID, int cnt, String status, int bnum) {
		...
		g.fillRect(                        10,  ID * 15 + 45 - 12 , width - 20, 15);
		g.setColor( c );
		g.setFont(new Font("verdana", Font.BOLD, 10));
		g.drawString(String.valueOf(ID),   20,                 ID * 15 + 45);
		g.drawString(String.valueOf(cnt),  20 + unit,          ID * 15 + 45);
		g.drawString(status,               20 + 2 * unit,      ID * 15 + 45);
		g.drawString(String.valueOf(bnum), 20 + 3 * unit + 20, ID * 15 + 45);
	}
...
class BurgerTrayPanel extends JPanel {
	...
	public void paintComponent(Graphics g)
	{
		...
		int width = getWidth();
		g.drawRect ((width - 200)/2 - 1, 35 - 1, 202, 10);
	...
	public void drawPCTBar(Graphics g, int pct)
	{
		...
		g.fillRect((width - 200)/2 , 35, 200, 9);
		g.setColor(Color.blue);
		g.fillRect((width - 200)/2, 35, 200*pct/100, 9);
		g.setColor(getBackground());
		g.fillRect((width-200)/2+210, 42 - 11, 55, 13);
		g.setColor(Color.blue);
		g.drawString(pct + "%", (width-200)/2+210, 44);
	}
	...
	public void drawItem(Graphics g, int[] tray, int idx)
	{
		...
		g.setColor(getBackground());
		g.fillRect(50 + unit * idx, 60 - 14, unit, 20);
		g.setFont( new Font("verdana", Font.BOLD, 10));
		if ( tray[idx] != 0 ) g.setColor(Color.BLUE);
		else g.setColor(Color.red);
		g.drawString(String.valueOf(tray[idx]), 50 + unit * idx, 60);
	}
...
class PCMonitor {
	...
	void printProducerConsumerTitle( JPanel panel, Color c, String msg) {
		Graphics g = panel.getGraphics();
		g.setFont( new Font("verdana", Font.BOLD, 10));
		// Eraze old message.
		g.setColor( panel.getBackground() );
		g.fillRect( 10, panel.getHeight() - 20 , panel.getWidth() - 20 , 12);

		// g.setColor( c );
		g.setColor( Color.black );
		g.drawString ( msg,  10, panel.getHeight() - 12 );
	}

	synchronized void printAddRemoveProducers (JPanel panel ) {
	     printProducerConsumerTitle( panel, Color.blue, 
		"Producers [added: " + PCLib.addedPro + ",  removed: " + PCLib.removedPro + "]");
	}

	synchronized void  printAddRemoveConsumers( JPanel panel ) {
	     printProducerConsumerTitle( panel, Color.red, 
		"Consumers [added: " + PCLib.addedCon + ",  removed: " + PCLib.removedCon + "]"); 
	}
...
class Producer extends Thread
{
	...
	public void drawCurrent(Graphics g)
	{
		PCLib.drawOneLine(panel, Color.blue, pID, total, status, bnum);
	}
	...
	{	PCLib.addedPro ++;
		...
		{
			making();
			PCLib.drawOneLine(panel, Color.blue, pID, total, "On queue", bnum);
...
class Consumer extends Thread {
	...
	public void run()
	{
		...
		{
			working();
			PCLib.drawOneLine(panel, Color.red, cID, burgersConsumed, "On queue", bnum);
			try { sleep(500); } catch (Exception e) {}
	...
	public void drawCurrent(Graphics g)
	{
		PCLib.drawOneLine(panel, Color.red, cID, burgersConsumed, status, bnum);
	}
...
class PCManager {
	...
	public void speedUp()
	{
		...
		PCLib.CONSUMERTIME = PCLib.CONSUMERTIME <= 0 ? ProducerConsumer2010.pnlWidth: PCLib.CONSUMERTIME; 
	}
