...
public class JumpingJava2D2 extends CloseableJFrame {
    JPanel drawingPane = new JPanel();
    static String msg = "Java 2D";
    public JumpingJava2D2( String title ) {
	super(title);
	...
	new JumpingJava2D2("Welcome to Java 2D");
    }
}
...
class ShowOneChar extends Thread {
	...
	ShowOneChar(JPanel p1, String m, int dx, Font fnt, WaitingCount w) {
		p = p1; g = (Graphics2D) p.getGraphics(); wc = w;
		f = fnt; msg = m; idx = dx;
		g.setFont( f );
	...
	public void run() {
	  ...
	  while ( true) {
		...
		clr = new Color( (int) (Math.random() * 256), (int)(Math.random() * 256),
			(int)(Math.random() * 256));
		g.setXORMode(clr);
